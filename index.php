...
$eventsFile = __DIR__ . '/src/data/eventos.json';
$eventsData = json_decode(file_get_contents($eventsFile), true);

// Filter available future events
$today = new DateTimeImmutable('today');
$availableFutureEvents = array_filter($eventsData, function ($event) use ($today) {
    try {
        $eventDate = new DateTimeImmutable($event['data_horario']);
        return $event['quantidade_disponivel'] > 0 && $eventDate >= $today;
    } catch (Exception $e) {
        error_log("Invalid date for event {$event['id']}: {$event['data_horario']}");
        return false;
    }
});

// Sort events by date ascending
usort($availableFutureEvents, function ($a, $b) {
    try {
        $dateA = new DateTimeImmutable($a['data_horario']);
        $dateB = new DateTimeImmutable($b['data_horario']);
        return $dateA <=> $dateB;
    } catch (Exception $e) {
        return 0;
    }
...
$featuredEventsForCarousel = array_slice($availableFutureEvents, 0, 5);
?>
<!DOCTYPE html>
<html lang="pt-BR">
...
<body>
    <main class="container py-4">
        <?php if (count($featuredEventsForCarousel) > 0): ?>
            <section class="mb-4 mb-md-5">
                <div id="featuredEventsCarousel" class="carousel slide" data-bs-ride="carousel">
                    <div class="carousel-inner">
                        <?php foreach ($featuredEventsForCarousel as $index => $event): ?>
    ...
    <script>
        ...
        const eventsData = <?= json_encode($availableFutureEvents, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
    </script>
</body>
